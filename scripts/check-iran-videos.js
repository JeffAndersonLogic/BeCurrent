...
for (const [topicId, topic] of topics) {
  if (!Array.isArray(topic.videos) || topic.videos.length < 2 || topic.videos.length > 3) {
    fail(`${topicId} should define 2–3 video/resource cards`);
    continue;
  }
  for (const [i, v] of topic.videos.entries()) {
    const tag = `${topicId} watch ${i + 1}`;
    for (const key of ['label','status','source','runtime','title','url','why','after','verified']) {
      if (!String(v[key] || '').trim()) fail(`${tag} missing ${key}`);
    }
...
if (!live) {
  ...
  console.log(`✓ Iran video metadata passes static launch checks (${rows.length} cards; audited ${catalog.reviewed}).`);
  console.log('  Run: node scripts/check-iran-videos.js --live');
  console.log('  Add --required-only to check only REQUIRED / REQUIRED EXCERPT cards.');
  process.exit(0);
}

async function checkOne(row){
  if (!external(row.url)) return {row, ok:true, status:'local'};
...
(async()=>{
  ...
  for (const result of results) {
    const label = `${result.row.topicId} W${result.row.index} ${result.row.status}`;
