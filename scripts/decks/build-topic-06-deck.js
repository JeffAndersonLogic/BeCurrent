...
(function guards() {
  ...
  if (T.slug !== 'who-decides') fail('topic ' + TOPIC_N + ' is no longer who-decides');
  ...
  ['You decide', 'The company decides', 'The government decides'].forEach((h, i) => {
    if (plain(T.sections[i + 1].heading) !== h) fail('section ' + (i + 1) + ' is no longer "' + h + '"');
  });
  ...
  if (!T.takeaway.includes('state the other side well enough that they would recognize it')) {
...
const POSITIONS = [
  { ord: 'Position one', heading: plain(T.sections[1].heading),
    strongest: [
  ...
  { ord: 'Position two', heading: plain(T.sections[2].heading),
    strongest: [
  ...
  { ord: 'Position three', heading: plain(T.sections[3].heading),
    strongest: [
...
const pres = K.newDeck({
  title: plain(T.title) + ', Social Media Topic ' + TOPIC_N,
...
/* ── 01. Title ──────────────────────────────────────────────────────────────── */
{
  ...
  K.body(s, plain(T.deck), { x: M, y: 2.5, w: 7.5, h: 0.9, face: F.display, size: 17, italic: true,
...
/* ── 02. Where this sits ────────────────────────────────────────────────────── */
{
  ...
  // Titles come from the topics themselves. A hand-typed list here went stale the
  // day Topic 4 was retitled, and nothing said so.
  const RECAP = {
    1: 'You built the chain from opening an app to a company getting paid.',
    2: 'You took the film apart the way you took your app apart.',
    3: 'You opened the box almost every chain had in the middle.',
    4: 'You read the evidence about effects, and what a record keeps.',
    5: 'You found out what that machine rewards, and who builds for it.'
  };
  const arc = UNIT.topics.filter(t => t.n < TOPIC_N)
    .map(t => [String(t.n), plain(t.title), RECAP[t.n]]);
  if (arc.length !== 5) throw new Error('expected 5 earlier topics, found ' + arc.length);
  arc.forEach(([n, label, text], i) => {
    const y = 1.6 + i * 0.54;
    K.chip(s, n, { x: M, y: y + 0.02, d: 0.34, size: 9.5, fill: P.slate700 });
    K.body(s, label, { x: 1.08, y, w: 2.9, h: 0.3, size: 12, bold: true, color: P.ink, valign: 'middle' });
    K.body(s, text, { x: 4.08, y, w: 5.3, h: 0.3, size: 12, valign: 'middle' });
  });
  ...
  K.body(s, plain(T.support[1].body), { x: 1.62, y: 4.3, w: 7.5, h: 0.64, size: 12,
...
/* ── 10. Precedent ────────────────────────────────────────────────────────── */
{
  ...
  K.title(s, plain(T.sections[4].heading), { size: 30 });
