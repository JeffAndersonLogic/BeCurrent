...
function workArea(slot, prompt) {
  ...
  const buttons = [1, 2, 3, 4, 5].map(n =>
    `<button type="button" data-conf="${n}" aria-pressed="${conf === String(n) ? 'true' : 'false'}"
      aria-label="Confidence ${n} of 5" onclick="setConfidence('${slot}', '${n}')">${n}</button>`
  ).join('');
  ...
  return `
    ...
    <div class="confidence" id="conf-${slot}">
...
// good one. A blank paste that still carries all eight headings reads as
// "student wrote nothing" when the real cause is a wiped localStorage.
//
// The footer declares how many slots the week defines, how many were gathered,
// and a hash per response, so scripts/parse-canvas-submissions.js can raise an
// exception instead of silently recording a blank.
//
...
// parser serves both courses. The sentinel below is not, because that line is
// visible in the student's paste. See the header of canvas-parse-core.js.
...
function bcHash(value) {
  const s = bcNormalizeForHash(value);
  let h = 0x811c9dc5;
...
function buildRecordManifest(work, isoStamp) {
  ...
  const sum = bcHash(rows.map(r => r.slot + ':' + r.responseHash).join('|'));

  const header = '#BHV|v=' + BC_RECORD_VERSION
    + '|topic=' + bcField(weekKey())
    + '|copied=' + isoStamp
    + '|items=' + rows.length
...
function paragraphsHtml(text) {
  ...
  if (!parts.length) return '<p><em>No response recorded.</em></p>';
...
function buildGatherDocument() {
  ...
  const manifest = buildRecordManifest(work, stamp.toISOString());
  const footer = '<hr>' + manifest.map(line =>
    `<p style="font-family:monospace;font-size:.68rem;opacity:.6;margin:.15rem 0;">${esc(line)}</p>`
  ).join('');
